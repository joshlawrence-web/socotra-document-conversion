...
| 9 | Coverage with `coverageTerms: [...]` | `coverageTerms: [{ name: "deductible", options: […] }]` | no | **yes** (coverage `terms` block, since 2026-06-29 — `$<iter>.<Cov>.<term>.value`) | partial (registry resolves the leaf; the `*`-default still trips the suggester — row 10) | `coverage_terms` | `conformance/fixtures/coverage-terms/` (C4, 2026-04-22; golden updated 2026-06-29) | No CommercialAuto coverage has a `coverageTerms` key (greps for `coverageTerms` return zero hits under `socotra-config/`). The extractor now reads `coverageTerms` two ways — string names into `coverageTerms/config.json` (with quantifier suffix) and inline `{name, value\|options}` objects — and emits one `terms` entry per term (`field: lowerFirst(name)`, `velocity: $<iter>.<Cov>.<term>.value`). C4 fixture uses `Flood.coverageTerms[deductible].options: ["250", "*500", "1000"]`: `data.effectiveDate` and the `deductible` term now both emit. `flood_deductible` resolves against the registry term; its `*`-default option remains a `default_option_prefix` concern for the suggester (row 10). |
