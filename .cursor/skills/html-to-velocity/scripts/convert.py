...
def wrap_conditionals(soup, mapping: Mapping):
    tbd_pattern = re.compile(r"\$TBD_([a-zA-Z_][a-zA-Z0-9_]*(?:\.[a-zA-Z_][a-zA-Z0-9_]*)*)")
    seen: set[int] = set()
...
def write_conditional_registry(blocks: list[dict], yaml_path: Path) -> None:
    ...
    header = (
        "# conditional-registry.yaml — auto-generated by Leg 1\n"
        "# Fill in 'conditions' (Java boolean expressions) and 'operator' (AND/OR) before running Leg 4.\n"
        "# Nested blocks (depth > 0) belong inside their parent's if-block — see parent_id.\n\n"
    )
    yaml_path.write_text(header + _yaml.dump(rows, allow_unicode=True, sort_keys=False), encoding="utf-8")


def write_conditional_review_md(blocks: list[dict], stem: str, md_path: Path) -> None:
    lines = [
        f"# Conditional Block Review — {stem}",
        "",
        "Auto-generated by Leg 1. Edit `conditions` in the `.yaml` file before running Leg 4.",
        "Child blocks (depth > 0) must be guarded inside their parent's if-block.",
        "",
        "| id | source_text | depth | parent | conditions | operator | notes |",
        "|----|-------------|-------|--------|------------|----------|-------|",
    ]
    for b in blocks:
        parent = f"cond{b['parent_id']}" if b.get("parent_id") else "—"
        src = b["source_text"][:60] + ("…" if len(b["source_text"]) > 60 else "")
        lines.append(f"| {b['id']} | {src} | {b.get('depth', 0)} | {parent} | *(fill in YAML)* | AND | |")
    md_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def convert(
    ...
    # 5.5 — extract conditional blocks ([prose text] → $doc.condN)
    ...
    cond_yaml_path = target_dir / f"{stem}.conditional-registry.yaml"
    write_conditional_registry(mapping.conditional_blocks, cond_yaml_path)
    ref_path = None
    cond_review_path = None
    if mapping.conditional_blocks:
        ref_path = target_dir / f"{stem}.conditional-ref.html"
        write_conditional_ref_html(vm_text, mapping.conditional_blocks, ref_path)
        cond_review_path = target_dir / f"{stem}.conditional-review.md"
        write_conditional_review_md(mapping.conditional_blocks, stem, cond_review_path)

    return vm_path, yaml_path, report_path, cond_yaml_path, ref_path, len(mapping.variables), len(mapping.loops), len(mapping.conditional_blocks)
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if args.batch:
        ...
        for html_path in args.batch:
            ...
            vm_path, yaml_path, report_path, _csv, _ref, var_count, loop_count, cond_count = convert(
                html_path,
                out_dir,
    ...
    if args.input:
        ...
        vm_path, yaml_path, report_path, csv_path, ref_path, _vars, _loops, _conds = convert(
            args.input,
            out_dir,
            with_conditionals=not args.no_conditionals,
            auto_detect=args.auto_detect_loops,
        ...
        if ref_path:
            print(f"Wrote {ref_path}  ({_conds} conditional block{'s' if _conds != 1 else ''})")
        return 0
